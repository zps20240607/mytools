@echo off
chcp 936 >nul
title RepoWatch 仓库总控台
cd /d "%~dp0"
echo ============================================
echo    RepoWatch 仓库总控台 - 启动中...
echo ============================================
...
powershell -NoProfile -Command "$pidFile='data\server.pid'; $p=Get-Content -LiteralPath $pidFile -ErrorAction SilentlyContinue; if($p){Stop-Process -Id ([int]$p) -Force -ErrorAction SilentlyContinue}; Remove-Item -LiteralPath $pidFile -Force -ErrorAction SilentlyContinue; 9610..9619 | ForEach-Object { Get-NetTCPConnection -State Listen -LocalPort $_ -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess } | Sort-Object -Unique | ForEach-Object { Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue }; Start-Sleep -Milliseconds 300"
start "" pythonw "%~dp0repo_watch.py" --no-browser %*
timeout /t 3 /nobreak >nul
set OK=0
set OKPORT=
for /L %%p in (9610,1,9619) do (
