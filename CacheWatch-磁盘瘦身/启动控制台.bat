...
powershell -NoProfile -Command "$pidFile='data\server.pid'; $p=Get-Content -LiteralPath $pidFile -ErrorAction SilentlyContinue; if($p){Stop-Process -Id ([int]$p) -Force -ErrorAction SilentlyContinue}; Remove-Item -LiteralPath $pidFile -Force -ErrorAction SilentlyContinue; 9630..9639 | ForEach-Object { Get-NetTCPConnection -State Listen -LocalPort $_ -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess } | Sort-Object -Unique | ForEach-Object { Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue }; Start-Sleep -Milliseconds 300"
